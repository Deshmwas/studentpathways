...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen font-sans bg-gray-50 pt-20">
      {/* Fixed Header */}
      <header className="fixed top-0 left-0 right-0 z-50 h-20 bg-black text-white flex items-center justify-between px-6 py-4 shadow-sm">
        <div className="flex items-center gap-6">
          <Image src="/logo.png" alt="Logo" width={40} height={40} className="rounded-md object-cover" />
          ...
          <div className="relative" ref={modulesRef}>
            ...
            {showModules && (
              <div className="absolute z-50 mt-2 bg-white text-black p-4 rounded shadow-lg grid grid-cols-2 gap-4 w-80">
                <h4 className="col-span-2 text-sm font-semibold text-gray-700">Add-on Modules</h4>
                {moduleLinks.map(({ label, icon }) => (
                  <div key={label} className="flex items-center gap-2 text-sm hover:text-indigo-600 cursor-pointer">
                    {icon}
          ...
          <div className="relative" ref={campusRef}>
            ...
            {showCampusDropdown && (
              <div className="absolute left-0 mt-2 w-72 bg-white text-black rounded shadow-md py-2 z-50">
                {campuses.map((campus) => (
                  <div
                    key={campus}
                    onClick={() => {
          ...
          <div className="relative" ref={yearRef}>
            ...
            {showYearDropdown && (
              <div className="absolute left-0 mt-2 w-40 bg-white text-black rounded shadow-md py-2 z-50">
                {years.map((year) => (
                  <div
                    key={year}
                    onClick={() => {
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <main className="flex-1 p-6 pt-4 overflow-y-auto bg-gray-50 min-h-screen">
          {children}
        </main>
      </div>
    </div>
